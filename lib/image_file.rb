...
class ImageFile
  ...
  def move from_dir, dest_numberOnly, dest_nameAndNumber, dest_nameOnly, index, indexes
    num = indexes[index]
    puts "index:#{index}, num:#{num}"
    xxx = "cp \"#{from_dir}#{@tempFilename}\" \"#{dest_nameAndNumber}#{'%03d' % num} - #{@personName} - #{@title}.jpg\""
    #    puts xxx
    system xxx
    xxx = "cp \"#{from_dir}#{@tempFilename}\" \"#{dest_nameOnly}#{@personName} - #{@title}.jpg\""
    #    puts xxx
    system xxx
    xxx = "mv \"#{from_dir}#{@tempFilename}\" \"#{dest_numberOnly}#{'%03d' % num}.jpg\""
    #    puts xxx
    system xxx

    # clear all metadata
    xxx = "exiftool -all= \"#{dest_numberOnly}#{'%03d' % num}.jpg\""
    # puts xxx
    system xxx
    xxx = "rm \"#{dest_numberOnly}#{'%03d' % num}.jpg_original\""
    system xxx
...
#    puts "moving"
    @file_info.each do |imageFile|
      imageFile.move(from_dir, dest_numberOnly, dest_nameAndNumber, dest_nameOnly, index, indexes)
      index += 1
    end
#    puts "moving-done"
    index
...
#puts "person_name2: #{person_name} - person: #{person}"
  ...
  def get_sorted_files
    num = @people_files.size
    @people_files.sort_by { |k, v| v.file_info[0].time }
  end

  def move source, dest_numberOnly, dest_nameAndNumber, dest_nameOnly
    total_files = 0;
    @people_files.each { |arr| total_files += arr[1].file_info.size }
    indexes = random((1..total_files).to_a, total_files)
    indexes2 = random(indexes, total_files)
    i = 0
    sorted_people_files = get_sorted_files
    sorted_people_files.each do |arr|
      i = arr[1].move(source, dest_numberOnly, dest_nameAndNumber, dest_nameOnly, i, indexes2)
      # puts "i=#{i}"
    end
  end

  def random rootArr, total_files

    indexes = Array.new(total_files)
    rootArr.each do |index|
      # puts(index)
      num = rand(total_files)+1
      while indexes.find_index { |x| x==num } != nil do
        num = rand(total_files)+1
      end
      indexes[index-1] = num
      # puts "indexes(#{index}) = #{indexes}"
    end
